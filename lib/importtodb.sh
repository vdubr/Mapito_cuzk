...
ogr2ogr -f "PostgreSQL" PG:"host=localhost user="$user "dbname="$db" password="$pass"" $filename"."$iformat -nlt $itype -nln $tablename -s_srs "epsg:"$isrs -t_srs "epsg:"$osrs -skipfailures 
